...
<script>
  	var width = $("#continer").width();
		$(function(){
			...
			if(flag=='4'){			
				$('#test').datagrid({
					...
					nowrap: false,
					striped: true,
					url:'agency/queryMerchModify?flag='+flag,
					remoteSort: false,
					columns:[
					[
		...
		function toMerchModifyEdit(id){
			window.location.href= "<%=basePath%>" +'agency/toMerchModifyEdit?merchApplyId='+id;
			//window.event.returnValue = false;
		}
		//初审、复审 
		function toMerchModifyAudit(merchApplyId){
			flag=$('#flag_ins').val();
			window.location.href= "<%=basePath%>" +'agency/toMerchModifyDetail?merchApplyId='+merchApplyId+'&flag='+flag;
			//window.event.returnValue = false;
		}
		...
		function toUpload(merchApplyId){
			window.location.href= "<%=basePath%>" +'/agency/toUploadModifyInfo?merchApplyId='+merchApplyId;
		}
